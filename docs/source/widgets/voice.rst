...
``local``
    ...
    ``pip install "djgentelella[asr]"``. The model (~670 MB) downloads from
    Hugging Face on first use.

``remote``
    Forwards the audio to an external ASR HTTP API
    (``GENTELELLA_ASR_REMOTE_URL``) via ``pip install
    "djgentelella[asr-remote]"``, which pulls an HTTP client and nothing else,
    presenting
