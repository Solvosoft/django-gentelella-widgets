...
ASYNC_NOTIFICATION_BACKEND = getattr(settings, "ASYNC_NOTIFICATION_BACKEND", None)
...
ASYNC_NOTIFICATION_MAX_RETRIES = getattr(settings, "ASYNC_NOTIFICATION_MAX_RETRIES", 3)

# Base delay (seconds) between retries; grows exponentially per attempt
ASYNC_NOTIFICATION_RETRY_DELAY = getattr(settings, "ASYNC_NOTIFICATION_RETRY_DELAY", 60)

# Custom recipient resolvers registered by suffix:
# {'suffix': 'dotted.path.ResolverClass', ...}
ASYNC_NOTIFICATION_RESOLVERS = getattr(settings, "ASYNC_NOTIFICATION_RESOLVERS", {})
...
ASYNC_NEWS_BASE_MODELS = getattr(settings, "ASYNC_NEWS_BASE_MODELS", {})

# Newsletter header configuration: {'logo': ..., 'title': ..., ...}
ASYNC_NEWSLETTER_HEADER = getattr(settings, "ASYNC_NEWSLETTER_HEADER", {})
...
ASYNC_NOTIFICATION_GROUP_LOOKUP_FIELDS = getattr(
    settings, "ASYNC_NOTIFICATION_GROUP_LOOKUP_FIELDS", ["name"]
)
...
ASYNC_NOTIFICATION_BASE_URL = getattr(settings, "ASYNC_NOTIFICATION_BASE_URL", "")
...
ASYNC_NOTIFICATION_LIST_ID = getattr(settings, "ASYNC_NOTIFICATION_LIST_ID", None)
...
ASYNC_NOTIFICATION_RATE_LIMIT = getattr(settings, "ASYNC_NOTIFICATION_RATE_LIMIT", 0)
