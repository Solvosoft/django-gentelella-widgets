...
class EmailNotificationAPITest(AsyncNotificationAPITestBase):
    ...
    def test_create_single_string_recipient(self):
        ...
        self.assertEqual(notification.bcc, ["boss@example.com", "other@example.com"])
    ...
    def test_retrieve(self):
        ...
        self.client.force_login(self.superuser)
        url = reverse(
            "async_notification:api-emailnotification-detail", args=[notification.pk]
        )
    ...
    def test_get_values_for_update(self):
        notification = EmailNotification.objects.create(
            subject="Old", message="M", recipients="a@b.com", base_template="product"
        )
        self.client.force_login(self.superuser)
        url = reverse(
        ...
        self.assertEqual(response.json()["base_template"]["id"], "product")
    ...
    def test_delete(self):
        ...
        self.assertFalse(EmailNotification.objects.filter(pk=notification.pk).exists())
    ...
    def test_send_email_action(self):
        ...
        self.client.force_login(self.superuser)
        url = reverse(
            "async_notification:api-emailnotification-send-email",
            args=[notification.pk],
        )
    ...
    def test_send_selected_action(self):
        ...
        response = self.client.post(
            url, {"pks": [n1.pk, n2.pk]}, content_type="application/json"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_duplicate_action_resets_to_pending(self):
        ...
        self.client.force_login(self.superuser)
        url = reverse(
            "async_notification:api-emailnotification-duplicate", args=[notification.pk]
        )
        response = self.client.post(url)
        self.assertEqual(response.status_code, 201, response.content)
