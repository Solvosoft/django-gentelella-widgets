...
class TrashViewSet(AuthAllPermBaseObjectManagement):
    ...
    def restore(self, request, org_pk=None, pk=None):
        # Outside the try: a missing entry is a 404, not a failed restore. The
        # catch-all below used to swallow this Http404 and answer 400 with
        # "could not be restored", which is the wrong status and the wrong
        # reason for the commonest case -- someone else already restored it
        # from another tab. DRF's exception handler turns it into
